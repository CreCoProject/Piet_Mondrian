...
void verticalSplit(int x1, int x2, float braight, int z)
{
  ...
  if( z < 6)
  {
    ...
    switch(splitParameter)
    {
      ...
      case 1:
        verticalSplit(x1, x1 + diff, braight, z);
        verticalSplit(x1 + diff, x2, braight, z);
        break;
      // Mode to make a Upper Division
      case 2:
        verticalSplit(x1, x2-diff, braight, z);
        verticalSplit(x2-diff, x2, braight, z);
        break;
    }
  }
  else
...
void horinzontalSplit(int x1, int x2, float braight, int z)
{
    ...
    // Process the Vertical Slit Mode
  {
    switch(splitParameter)
    {
      ...
      // Mode to make a Lower Division
      case 1:
        horinzontalSplit(x1, x1+diff, braight, z);
        horinzontalSplit(x1+diff, x2, braight, z);
        break;
      // Mode to make a Upper Division
      case 2:
        horinzontalSplit(x1, x2-diff, braight, z);
        horinzontalSplit(x2-diff, x2, braight, z);
        break;
    }
  }
  else
